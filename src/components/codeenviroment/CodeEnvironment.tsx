...
const CodeEnvironment: React.FC<CodeEnvironmentProps> = ({
  ...
  const makeApiRequest = async (data: {
    language: string;
    version: string;
    files: { content: string }[] | { content: string }[];
  }) => {
  ...
  const runTests = async (
    ...
    if (!selectedProblem || !input.trim()) {
      const errorMsg =
        'Please write some code and select a problem before running tests.';
      setErrors(errorMsg);
    ...
    const customCases = customTestCases.filter(
      (tc) => tc.isCustom && tc.input.trim(),
    );
    ...
    for (let i = 0; i < allTestCases.length; i++) {
      ...
      try {
        ...
        if (output) {
          ...
          const cleanExpected = cleanDisplayText(
            testCase.expected || '',
          ).replace(/\s+/g, '');
      ...
      updateConsole(
        setConsoleOutput,
        `Test Case ${i + 1}: ${status} (${execTime}ms)`,
      );
    }
      ...
      <PanelGroup direction="horizontal" className="flex-1">
        ...
        {!isProblemPanelCollapsed && (
          <>
            <Panel minSize={25} defaultSize={50} maxSize={70}>
              <div className="relative flex h-full flex-col bg-white">
                ...
                <button
                  ...
                  className="absolute right-4 top-4 z-10 rounded-md bg-gray-100 p-2 text-black transition-colors hover:bg-gray-200"
                ...
                <div className="flex-1 overflow-auto">
                  <div className="p-6 pr-16">
                    ...
                    <div className="mb-6">
                      <ProblemDisplay problem={selectedProblem} className="" />
                    </div>
            ...
            <PanelResizeHandle className="w-2 cursor-col-resize bg-gray-200 hover:bg-gray-300" />
          </>
        )}

        {/* Show Problem Button (when collapsed) */}
        {isProblemPanelCollapsed && (
          <div className="flex w-12 flex-col items-center justify-start border-r border-gray-200 bg-gray-50 pt-4">
            <button
              onClick={() => setIsProblemPanelCollapsed(false)}
              className="mb-4 rounded-md bg-blue-100 p-3 text-blue-700 transition-colors hover:bg-blue-200"
