...
export const createCustomProblem = (
...
export const updateCustomProblem = (
  id: string,
  updates: Partial<NewProblemData>,
): CustomProblem => {
  const existingProblems = getCustomProblems();
  ...
  const updatedProblem: CustomProblem = {
    ...
    slug: updates.title
      ...
      : existingProblems[problemIndex].slug,
  };
...
export const importCustomProblems = (
  jsonData: string,
  ...
  try {
    ...
    importedProblems.forEach((problem, index) => {
      if (!problem.title || !problem.description || !problem.difficulty) {
        throw new Error(
          `Invalid problem at index ${index}: missing required fields`,
        );
      }
    });
    ...
    importedProblems.forEach((importedProblem) => {
      const exists = mergedProblems.some(
        (existing) =>
          existing.title.toLowerCase() === importedProblem.title.toLowerCase(),
      );
